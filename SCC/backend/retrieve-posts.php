...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "	<div class='card-body'>
				<h5 class='card-title font-weight-bold'>" . $row['first_name'] . " " . $row['last_name'] . "<small class='font-weight-light pl-2'>" . $row['upload_date'] . "</small></h5>";

        if ($row['post_content'] != null) {
            echo "<p class='card-text'>" . $row['post_content'] . "</p>";
        }
        echo "<form method='POST' action=../backend/add-comment.php>
		  <textarea class='form-control' rows='2' name='comment' id='comment' placeholde='comment'></textarea>
